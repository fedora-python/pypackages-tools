...
from pptools import load_cf_parsers, path_norm_join, PYTHON_LIBDIRS
...
class ByteCompileConfig(object):
    ...
    @classmethod
    def from_cf_parser(cls, name, parser):
        """A class method that instantiates and returns a config from given parser."""
        items = []
        if parser.has_section('bytecompile'):
            items = parser.items('bytecompile')
        return cls(name, **dict(items))
...
def bytecompile(rpm_buildroot, default_python, errors_terminate, config_dir, dry_run):
    ...
    if rpm_buildroot == '/':
        return 0
    parsers = load_cf_parsers(config_dir)
    configs = {n: ByteCompileConfig.from_cf_parser(n, p) for n, p in parsers.items()}
    ...
    if dry_run:
        for fname, run_strings in to_run.items():
            logger.info('Running from config "{0}":'.format(fname))
            [logger.info(rs) for rs in sorted(run_strings)]
    else:
...
def unassoc_libdirs_errors(configs, rpm_buildroot):
    ...
    if not_matched:
        logger.error('Error: there are Python libdirs not associated with any Python runtime:')
        [logger.error(nm) for nm in sorted(not_matched)]
        return True
...
def compile_roots_errors(configs):
    ...
    if errs:
        logger.error('Error, following roots are to be compiled by multiple Pythons:')
